...
struct ProfileView: View {
    ...
    private var planUsageLabel: String {
        ...
        // Gate on tier too, not just the server flag: `isUnlimited` defaults
        // false, so a Pro response that omits it must still read "Unlimited".
        if subscriptions.isUnlimited || subscriptions.tier == .pro {
            return "Unlimited"
        }
